...
public class ArrayHeapMinPQ<T> implements ExtrinsicMinPQ<T> {
    ...
    public ArrayHeapMinPQ() {
        items = new ArrayList();
        items.add(new PriorityNode(0, 0.0));
    ...
    public T removeSmallest() {
        ...
        if (keys.size() == 1) {
            T item = (T) endNode.getItem();
            keys.remove(item);
            return item;
        }

        int fInd = 1;
        T item = (T) items.remove(fInd).getItem();
    ...
    private void swim(int k) {
        ...
        if (items.get(k).getPriority()
                < items.get(parent(k)).getPriority()) {
            swap(k, parent(k));
            swim(parent(k));
        }
    }
